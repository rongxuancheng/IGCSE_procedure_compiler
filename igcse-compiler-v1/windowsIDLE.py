    ...
    def parse_primary(self):
        ...
        if t.type=='ID':
            ...
            if uname in ('LENGTH','LEN'):
                self.expect('(','期望 ('); a=self.parse_expr(); self.expect(')','期望 )')
    ...
    def parse_stmt(self):
        ...
        if t.type=='DECLARE':
            ...
            while not self.check('ENDCASE','EOF'):
                ...
                self.emit(kw)
                self.indent+=1
                self.parse_block('CASE','OTHERWISE','ENDCASE')
                self.indent-=1
                self.skip_nl()
...
class IGCSEide(tk.Tk):
    ...
    def _run(self):
        ...
        self._stop_btn.config(state=tk.NORMAL, bg="#a81010")
